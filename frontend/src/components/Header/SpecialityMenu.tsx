  ...
  return (
    <section className="px-4 py-12 text-gray-800 sm:py-16">
      <div className="mx-auto max-w-7xl text-center">
        ...
        <div className="mt-8 flex gap-5 overflow-x-auto pb-4 scrollbar-hide lg:grid lg:grid-cols-10 lg:overflow-visible">
          {SpecialityData.map((item) => (
            <Link to={`/doctors/speciality/${encodeURIComponent(item.speciality)}`} className="flex min-w-[95px] flex-col items-center text-center text-sm"
              key={item.speciality}>
              <div className="flex h-20 w-20 items-center justify-center rounded-full bg-blue-400 p-4 shadow-md transition-all duration-300 hover:scale-105 sm:h-24 sm:w-24 sm:p-5">
