...
export const AddDoctor = () => {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      const departmentList = Array.isArray(rawDepartments)
        ? rawDepartments.map(normalizeDepartment)
        : [];
    ...
    const loadDoctorForEdit = async () => {
      ...
      try {
        ...
        if (!doctorData) {
          ...
          doctorData =res.data?.data?.doctor ||
            res.data?.doctor ||
            res.data?.data ||
            res.data;
        }
        ...
        const existingImage =
          doctorData.user?.profile_picture || doctorData.image;
      ...
      <main className="flex-1 p-8">
        <div className="mx-auto max-w-5xl rounded-2xl bg-white p-8 shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-6">
            ...
            <SelectBox icon={<FaUser />} label="Role" name="role" value={form.role} onChange={handleChange}
              options={["doctor"]} />
            ...
            <InputBox icon={<FaBriefcase />} label="Experience Years" name="experience_years" type="number"
