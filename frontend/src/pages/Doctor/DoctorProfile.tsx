...
import {FaCamera,FaCalendarAlt,FaEnvelope,FaSave,FaStar,FaStethoscope,FaTimes,FaUserMd,FaUsers,FaGraduationCap,} from "react-icons/fa";
...
export const DoctorProfile = () => {
  ...
  const fetchDoctor = async () => {
    try {
      ...
      const doctor =
        res.data?.data?.doctor || res.data?.doctor || res.data?.data || res.data;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-sky-100 lg:flex pt-20">
      <DoctorSidebar />
          ...
          <div className="relative z-10 flex flex-col items-center gap-8 text-center lg:flex-row lg:text-left">
            ...
            <div className="w-full flex-1 text-white">
              ...
              <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
                <InfoCard icon={<FaCalendarAlt />} title="Appointments" value="256" />
                <InfoCard icon={<MdWorkspacePremium />} title="Experience"
                  value={`${profile.experience_years || 0}+ Years`}/>
                <InfoCard icon={<FaUsers />} title="Patients" value="420+" />
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="space-y-8 p-5 sm:p-8">
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              ...
              <InputField label="Experience Years" name="experience_years" value={profile.experience_years} onChange={handleChange}
                icon={<FaCalendarAlt />}
                readOnly={!isEdit}/>
              <SpecializationDropdown value={profile.specialization} departments={departments} disabled={!isEdit} isOpen={specializationOpen}
                setIsOpen={setSpecializationOpen}
                onSelect={(value) =>
                  setProfile((prev) => ({ ...prev, specialization: value }))
                }/>
              <InputField label="Consultation Fee" name="consultation_fee" value={profile.consultation_fee} onChange={handleChange}
                icon={<FaStethoscope />}
                readOnly={!isEdit}/>
  ...
  <div className="relative">
    ...
    <button
      ...
      className={`flex h-14 w-full items-center justify-between rounded-2xl border px-4 text-left transition sm:h-16 ${
        disabled
          ? "cursor-not-allowed border-blue-100 bg-blue-50"
...
const InputField = ({ label, icon, readOnly, ...props }: any) => (
  <div>
    ...
    <div
      className={`flex h-14 items-center rounded-2xl border px-4 transition sm:h-16 ${
        readOnly
    ...
    >
      ...
      <input
        {...props}
        readOnly={readOnly}
        className="h-full min-w-0 flex-1 bg-transparent px-3 text-base font-semibold text-slate-800 outline-none"
      />
    </div>
  </div>
);