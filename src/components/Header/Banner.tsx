    ...
    <section className="bg-gradient-to-br from-blue-50 via-white to-indigo-50 px-4 py-10 sm:px-6 sm:py-14">
      ...
      <div className="mx-auto grid max-w-5xl items-center gap-6 md:grid-cols-2 lg:gap-10">
        <motion.div initial={{ opacity: 0, x: -60 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}
          viewport={{ once: true }} className="relative mx-auto flex w-full max-w-[340px] items-end justify-center py-8 px-12 sm:max-w-[380px]">
          <div className="absolute bottom-8 left-1/2 h-56 w-56 -translate-x-1/2 rounded-full bg-blue-100 sm:h-64 sm:w-64" />
          ...
          <motion.div animate={{ y: [0, -6, 0] }} transition={{ duration: 3, repeat: Infinity }}
            className="absolute right-0 top-2 z-20 flex items-center gap-2 rounded-2xl bg-white px-3 py-2 shadow-lg">
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-green-500 text-white">
              <FaCheckCircle className="text-xs" />
            </span>
            <div>
              <p className="text-[11px] font-extrabold text-gray-800">500+</p>
          ...
          <motion.img animate={{ y: [0, -10, 0] }} transition={{ duration: 4, repeat: Infinity }}
            src={doctorImg} alt="doctor" className="relative z-10 w-[200px] drop-shadow-xl sm:w-[240px]"/>
        </motion.div>
