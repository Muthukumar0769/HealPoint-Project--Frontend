  ...
  return (
    <>
      ...
      <section className="overflow-hidden bg-white px-4 py-12 sm:px-6 lg:py-16">
        <div className="mx-auto grid max-w-4xl items-center gap-8 md:grid-cols-2 lg:gap-12">
          <motion.div
            initial={{ opacity: 0, x: -80 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true }}
           className="relative mx-auto w-full max-w-xs lg:max-w-sm overflow-hidden p-6">
            <motion.div
              animate={{ rotate: 360 }}
              transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
              className="absolute inset-2 rounded-full border-4 border-dashed border-blue-200 sm:border-[6px]" />

            <div className="absolute left-0 top-0 h-32 w-32 rounded-full bg-blue-200 opacity-40 blur-3xl sm:h-40 sm:w-40" />
            <motion.img
              animate={{ y: [0, -18, 0] }}
