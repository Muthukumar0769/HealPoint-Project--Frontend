...
const StatCard = ({ label, value, icon, color }: {
  ...
  return (
    <div className={`rounded-2xl ${bg[color]} p-4`}>
      ...
      <main className="min-w-0 flex-1 overflow-hidden px-4 pb-10 pt-24 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm shadow-blue-100">
              {FILTERS.map((f) => (
                <button key={f} onClick={() => handleFilterChange(f)} className={`cursor-pointer px-4 py-2 text-xs font-extrabold transition ${filter === f ? "bg-blue-600 text-white" : "text-slate-500 hover:bg-blue-50 hover:text-blue-600"}`}>
                  {f.charAt(0).toUpperCase() + f.slice(1)}
                </button>
              ))}
            </div>
            <button onClick={() => navigate("/admin/earnings/doctors")} className="cursor-pointer flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-extrabold text-blue-600 transition hover:bg-blue-600 hover:text-white">
              <FaUserMd className="text-sm" /> Top Earning Doctors
            </button>
            <button onClick={handleExport} disabled={!data} className="flex cursor-pointer items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-xs font-extrabold text-white shadow-sm shadow-blue-200 transition hover:bg-blue-700 disabled:opacity-50">
        ...
        {!loading && data && summary && (
          <>
            ...
            <div className="mb-5 rounded-2xl bg-white p-5 shadow-lg shadow-blue-100">
              <div className="mb-4 flex items-start justify-between">
                <div>
                  <h2 className="text-sm font-extrabold text-slate-800">Revenue Trend</h2>
                  <p className="mt-0.5 text-xs text-slate-400">Video call earnings over time</p>
                </div>
                <div className="flex items-center gap-4 text-xs text-slate-500">
                  <span className="flex items-center gap-1.5">
                    <span className="h-2.5 w-2.5 rounded-sm bg-blue-600" /> Video Call
                  </span>
                </div>
              </div>
              <ResponsiveContainer width="100%" height={240}>
                <AreaChart data={data.trendData} margin={{ top: 4, right: 8, left: 8, bottom: 0 }}>
                  <defs>
            ...
            <div className="rounded-2xl bg-white p-5 shadow-lg shadow-blue-100">
              <div className="mb-4">
                <h2 className="text-sm font-extrabold text-slate-800">Recent Video Consultations</h2>
                <p className="mt-0.5 text-xs text-slate-400">Latest video call transactions</p>
              </div>
                        ...
                        <tr key={idx} className="border-b border-slate-100 transition hover:bg-blue-50/40">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <img src={row.patient_profile || `https://ui-avatars.com/api/?name=${encodeURIComponent(row.patient_name || "P")}&background=dbeafe&color=1d4ed8`} alt={row.patient_name}
                                className="h-8 w-8 flex-shrink-0 rounded-full object-cover" />
                              <span className="text-xs font-bold text-slate-800">{row.patient_name || "—"}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <img src={row.doctor_profile || `https://ui-avatars.com/api/?name=${encodeURIComponent(row.doctor_name || "D")}&background=dcfce7&color=166534`} alt={row.doctor_name}
                                className="h-8 w-8 flex-shrink-0 rounded-full object-cover" />
                              <div>
                                <p className="text-xs font-bold text-slate-800">{row.doctor_name || "—"}</p>
                                <p className="text-[10px] text-slate-400">{row.specialization}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-extrabold bg-blue-100 text-blue-700">
                              <FaVideo className="text-[9px]" /> Video Call
                            </span>
                          </td>
                          <td className="px-4 py-3 text-xs text-slate-500">
                            {row.consultation_date ? formatDate(row.consultation_date) : "—"}
                          </td>
                          <td className="px-4 py-3 text-right text-xs font-extrabold text-slate-800">
                            {formatINR(row.amount)}
                          </td>
                          <td className="px-4 py-3">
                            <span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${row.payment_status === "paid" ? "bg-green-100 text-green-700"
                              : row.payment_status === "pending" ? "bg-orange-100 text-orange-700"
                ...
                <div className="mt-5 flex items-center justify-between text-xs text-slate-500">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {Array.from({ length: totalTablePages }, (_, i) => i + 1).map((p) => (
                      <button key={p} onClick={() => goToTablePage(p)}
                        className={`h-8 w-8 cursor-pointer rounded-xl text-xs font-bold transition-all duration-200 ${p === tablePage ? "bg-blue-600 text-white" : "border border-slate-200 text-slate-600 hover:bg-slate-50"}`}>
                        {p}
                      </button>
                    ))}
