...
import { FaCheckCircle, FaTimesCircle, FaVideo, FaSpinner, FaChevronLeft, FaChevronRight, FaPlay, FaClock,
  FaRedoAlt, FaBan, FaStar, FaMapMarkerAlt, FaChevronDown, FaChevronUp,FaShieldAlt,FaLightbulb} from "react-icons/fa";
...
const getPatientAppointments = async (page = 1, limit = PAGE_SIZE): Promise<PaginatedResponse> => {
  ...
  const d = res.data;
  const raw = d?.appointments !== undefined ? d : d?.data?.appointments !== undefined ? d.data
...
const joinAppointmentAPI = async (id: number) => {
  const res = await API.get(`/appointments/${id}/join`);
  return res.data;
};

const cancelAppointmentAPI = async (id: number) => {
...
const formatTime = (time?: string) => {
  if (!time) return "N/A";
  return new Date(`2000-01-01T${time}`).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", hour12: true });
};
...
const isVideoConsultation = (a: MyAppointment) => {
  const type = String(a.consultation_type ?? (a as any).consultationType ?? (a as any).appointment_type ?? "").toLowerCase();
  return type.includes("video") || type.includes("online") || type.includes("virtual") || !!a.meeting_room;
};

const isDoctorMissed = (a: MyAppointment) => {
  const cs = String(a.consultation_status ?? "").toLowerCase();
  return cs === "no_show" || cs === "missed";
};

const isDoctorCancelled = (a: MyAppointment) => {
  const s = String(a.status ?? "").toLowerCase();
  return s === "cancelled" || s === "canceled" || s === "rejected";
};

const canJoin = (a: MyAppointment, now: Date): boolean => {
...
const DoctorAvatar = ({ appointment }: { appointment: MyAppointment }) => {
  ...
  const initials = getInitials(appointment.doctor?.name);
  if (!src || error) {
    return (
  ...
  return (
    <img src={src} alt={appointment.doctor?.name || "Doctor"} className="h-full w-full rounded-full object-cover object-top"
  ...
  return (
    <div className="flex items-center justify-center gap-2 mt-8 flex-wrap">
      <button onClick={() => onChange(currentPage - 1)} disabled={currentPage === 1}
        className="w-9 h-9 flex items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:bg-slate-50 hover:border-slate-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm cursor-pointer">
      ...
      {pages.map((p, i) =>
        p === "…" ? (
          ...
          <button key={p} onClick={() => onChange(p as number)}
            className={`w-9 h-9 flex items-center justify-center rounded-xl text-sm font-bold transition-all duration-200 cursor-pointer shadow-sm ${currentPage === p
              ? "bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-blue-200" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300"}`}>
            {p}
          </button>
        )
      )}
...
const CancelModal = ({ doctorName, loading, onConfirm, onClose }: {
  doctorName: string; loading: boolean; onConfirm: () => void; onClose: () => void;
}) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
    <div className="w-full max-w-sm rounded-3xl bg-white p-7 shadow-2xl border border-slate-100">
      <div className="mb-6 flex flex-col items-center gap-4 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-red-50 border border-red-100 text-2xl text-red-500 shadow-sm">
          <FaBan />
        </div>
        <div>
      ...
      <div className="flex gap-3">
        <button onClick={onClose} disabled={loading} className="h-11 flex-1 cursor-pointer rounded-2xl border-2 border-slate-200 text-sm font-bold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all">
          Keep It
        </button>
...
export const MyAppointments = () => {
  ...
  const handleJoinMeeting = async (appt: MyAppointment) => {
    ...
    try {
      ...
      if (!meetingRoom) { alert("Meeting room not available. Please try again."); return; }
      setActiveCall({ meetingRoom, doctorName: appt.doctor?.name ?? "Doctor", appointmentId: appt.id });
    } catch (err: any) {
      alert(err?.response?.data?.message ?? "Unable to join meeting. Please try again.");
    } finally {
      setJoinLoading(null);
    }
  };
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      await cancelAppointmentAPI(cancelModal.id);
      setCancelModal(null);
  ...
  const handleReschedule = (appt: MyAppointment) => {
    ...
    if (doctorId) navigate(`/doctors/doctor-details/book-appointment/${doctorId}`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 px-3 pb-10 pt-16 sm:px-4 sm:pt-20 lg:pt-24">
        ...
        <div className="flex gap-3">
          <div className="hidden lg:flex lg:flex-col lg:shrink-0 lg:gap-4 self-start sticky top-24" style={{ width: "200px" }}>
            <div className="rounded-3xl overflow-hidden relative shadow-xl"
              ...
              <div className="absolute inset-0 h-full w-full  bg-gradient-to-b from-blue-900/60 to-blue-950/80 rounded-3xl" />
              <div className="relative z-10 p-4 flex flex-col">
                ...
                <div className="flex justify-center my-2">
                  <img src={doctorImage} alt="Doctor" className="xl:w-[200px] lg:w-[140px] w-140px object-contain drop-shadow-2xl" />
                </div>
                <div className="space-y-2">
                  {[
                    { icon: "🏥", num: "10,000+", label: "Happy Patients" },
                    { icon: "👨‍⚕️", num: "500+", label: "Expert Doctors" },
                    ...
                    <div key={i} className="flex items-center gap-2 rounded-xl bg-white/10 backdrop-blur-sm px-3 py-2 border border-white/10">
                      ...
                      <div>
                        <p className="text-white font-extrabold text-xs leading-none">{item.num}</p>
          ...
          <div className="flex-1 min-w-0">
            <div className="mb-6 px-1">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  <h1 className="text-2xl font-extrabold leading-tight text-slate-900 sm:text-3xl tracking-tight">
                    My Appointments
                  </h1>
            ...
            <div className="mb-5 flex items-center gap-2 rounded-2xl bg-white px-3 py-2.5 shadow-sm border border-slate-100 overflow-x-auto no-scrollbar">
              {tabs.map(tab => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                  className={`flex shrink-0 cursor-pointer items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition-all duration-200 ${activeTab === tab.key
                    ? `bg-gradient-to-r ${tab.color} text-white shadow-md` : "text-slate-500 hover:bg-slate-50 hover:text-slate-700"}`}>
                  {tab.label}
                  <span className={`flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-xs font-extrabold transition-all ${activeTab === tab.key
                    ? "bg-white/25 text-white" : "bg-slate-100 text-slate-500"}`}>
                    {tab.count}
                  </span>
                </button>
              ))}
            ...
            <div ref={listRef} className="space-y-3.5">
              {loading && (
                <div className="flex flex-col items-center justify-center gap-4 rounded-3xl bg-white p-16 shadow-sm border border-slate-100">
              ...
              {!loading && filteredAppointments.length === 0 && (
                <div className="rounded-3xl bg-white p-16 text-center shadow-sm border border-slate-100">
                  ...
                  <p className="text-sm text-slate-300 mt-1">Book a new appointment to get started</p>
                  <button onClick={() => navigate("/doctors")} className="mt-5 inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white text-sm font-bold px-6 py-2.5 hover:from-blue-600 hover:to-indigo-700 transition-all shadow-sm cursor-pointer">
                    Find a Doctor →
                  </button>
                </div>
              )}
              ...
              {!loading && filteredAppointments.map(appt => {
                ...
                return (
                  <div key={appt.id}
                    className="rounded-3xl bg-white shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden border border-slate-100/80 group">
                    <div className="flex flex-col gap-0 sm:flex-row sm:items-stretch">
                      <div className={`flex flex-col items-center justify-center gap-0.5 shrink-0 w-[88px] py-4 bg-gradient-to-br ${accent} relative`}>
                        <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_top_right,white,transparent)]" />
                        <div className="relative z-10 text-center">
                          <span className="text-[11px] font-bold text-white/80 block">{getDayAbbr(appt.appointment_date)}</span>
                          <span className="text-4xl font-black text-white leading-none block">{getDayNum(appt.appointment_date)}</span>
                          <span className="text-[11px] font-bold text-white/80 block">{getMonthAbbr(appt.appointment_date)}</span>
                        </div>
                      </div>

                      <div className="flex flex-1 items-center gap-4 px-5 py-4">
                        <div className="h-14 w-14 shrink-0 overflow-hidden rounded-full ring-2 ring-slate-100 shadow-sm">
                          <DoctorAvatar appointment={appt} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-0.5">
                            <h3 className="text-base font-extrabold text-slate-900 tracking-tight">Dr. {doctorName}</h3>
                            <span className={`flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-bold ${statusCls}`}>
                              <span className={`h-1.5 w-1.5 rounded-full ${dot}`} />
                              {statusLabel}
                            </span>
                          </div>
                          <p className="text-xs font-semibold text-blue-500 mb-2">{specialization}</p>
                          <div className="flex flex-wrap items-center gap-4">
                            <span className="flex items-center gap-1.5 text-xs text-slate-500 bg-slate-50 rounded-lg px-2.5 py-1">
                              <FaClock className="text-blue-400 text-[10px]" />
                              <span className="font-semibold">{formatTime(appt.start_time)}</span>
                            </span>
                            <span className="flex items-center gap-1.5 text-xs text-slate-500 bg-slate-50 rounded-lg px-2.5 py-1">
                              {isVideoConsultation(appt) ? <FaVideo className="text-indigo-400 text-[10px]" />
                          ...
                          <div className="flex flex-wrap justify-end gap-2">
                            {showVideo && (
                              <button onClick={() => joinable ? handleJoinMeeting(appt) : undefined} disabled={!joinable || isJoining}
                                ...
                                className={`flex items-center gap-1.5 rounded-xl px-4 py-2 text-xs font-bold transition-all shadow-sm ${joinable && !isJoining
                                ...
                                {isJoining
                                  ...
                                  : <><FaPlay className="text-[9px]" />Join Call</>}
                              </button>
                            )}
                            {isConfirmed && (
                              <button onClick={() => handleReschedule(appt)} className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all cursor-pointer shadow-sm">
                                <FaRedoAlt className="text-[9px]" />Reschedule
                              </button>
                            )}
                            {isConfirmed && (
                              <button onClick={() => setCancelModal(appt)} className="flex items-center gap-1.5 rounded-xl border border-red-200 bg-red-50 px-3.5 py-2 text-xs font-bold text-red-500 hover:bg-red-100 transition-all cursor-pointer shadow-sm">
                                <FaTimesCircle className="text-[9px]" />Cancel
                              </button>
                            )}
                            {canReview && (
                              <button onClick={() => setReviewModal(appt)} className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 px-3.5 py-2 text-xs font-bold text-white hover:from-amber-500 hover:to-orange-600 active:scale-95 transition-all cursor-pointer shadow-sm">
                                <FaStar className="text-[9px]" />Review
                              </button>
                            )}
                            {appt.review_given && (
                              <span className="flex items-center gap-1.5 rounded-xl bg-emerald-50 border border-emerald-100 px-3.5 py-2 text-xs font-bold text-emerald-600">
                                <FaCheckCircle className="text-[9px]" />Reviewed
                              </span>
                            )}
                            {isCompleted && !isMissed && !canReview && !appt.review_given && (
                              <span className="flex items-center gap-1.5 rounded-xl bg-blue-50 border border-blue-100 px-3.5 py-2 text-xs font-bold text-blue-600">
                                <FaCheckCircle className="text-[9px]" />Completed
                              </span>
                            )}
                          </div>
                          <button onClick={() => setExpandedId(isExpanded ? null : appt.id)} className="flex items-center gap-1.5 text-xs font-bold text-blue-500 hover:text-blue-700 cursor-pointer transition-colors group-hover:underline">
                    ...
                    {isExpanded && (
                      <div className="border-t border-slate-100 bg-gradient-to-br from-slate-50 to-blue-50/30 px-6 py-5">
                        ...
                        {isCancelled && !isMissed && (
                          <div className="rounded-2xl border border-red-100 bg-red-50 px-5 py-4 flex flex-wrap items-center justify-between gap-3">
                            <div className="space-y-1.5">
                              <p className="flex items-center gap-2 text-xs font-bold text-red-500">
                                <FaTimesCircle />Appointment cancelled. Please choose another slot.
                              </p>
                              <p className="flex items-center gap-2 text-xs font-bold text-emerald-600">
                                <FaCheckCircle />Refund will be processed within 3–5 business days.
                              </p>
                            </div>
                            <button onClick={() => handleReschedule(appt)} className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 px-5 py-2.5 text-xs font-extrabold text-white hover:from-blue-600 hover:to-indigo-700 active:scale-95 transition-all cursor-pointer shadow-sm">
                        ...
                        {isMissed && (
                          <div className="rounded-2xl border border-orange-100 bg-orange-50 px-5 py-4 flex flex-wrap items-center justify-between gap-3">
                            <div className="space-y-1.5">
                              <p className="flex items-center gap-2 text-xs font-bold text-orange-500">
                                <FaVideo />You missed this appointment slot.
                              </p>
                              <p className="flex items-center gap-2 text-xs font-bold text-emerald-600">
                                <FaCheckCircle />Book a new slot to continue your care.
                              </p>
                            </div>
                            <button onClick={() => handleReschedule(appt)} className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-orange-400 to-amber-500 px-5 py-2.5 text-xs font-extrabold text-white hover:from-orange-500 hover:to-amber-600 active:scale-95 transition-all cursor-pointer shadow-sm">
                              <FaRedoAlt />Reschedule Now
                            </button>
                          </div>
                        )}
                        ...
                        {showVideo && !joinable && (
                          <div className="mt-3 flex items-center gap-2 rounded-xl bg-blue-50 border border-blue-100 px-4 py-2.5">
                            ...
                            <p className="text-xs text-blue-600 font-semibold">
                              Video session becomes available 10 minutes before your appointment time.
                            </p>
                          </div>
                        )}
                      </div>
            ...
            <div className="rounded-3xl bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700 p-4 shadow-lg overflow-hidden relative shrink-0">
              ...
              <div className="relative z-10 flex flex-col">
                ...
                <p className="text-white/70 text-[11px] leading-relaxed mb-3">
                  Book regular check-ups and stay ahead of your health with trusted specialists.
                </p>
                <button onClick={() => navigate("/doctors")} className="w-full rounded-xl bg-white text-blue-600 text-xs font-extrabold py-2.5 hover:bg-blue-50 transition-all cursor-pointer shadow-lg">
                  Book Now →
                </button>
              </div>
            </div>
            <div className="rounded-3xl bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-100 p-4 shadow-sm overflow-hidden relative shrink-0">
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-end gap-2">
                  <p className="text-slate-500 w-30 text-[11px] leading-relaxed flex-1">
                    A good night's sleep improves your mood, boosts immunity and keeps your heart healthy.
                  </p>
                  <img src={sleepImg} alt="Sleep" className="w-22 h-22 object-contain shrink-0 drop-shadow-sm" />
                </div>
              </div>
            </div>
            <div className="rounded-3xl overflow-hidden relative shadow-sm shrink-0" style={{ backgroundImage: `url(${greenBg})`, backgroundSize: "cover", backgroundPosition: "center", minHeight: "160px" }}>
              ...
              <div className="relative z-10 p-4 flex flex-col" style={{ minHeight: "160px" }}>
                ...
                <p className="text-white/75 text-[11px] leading-relaxed mb-2">
                  Consult top specialists and make confident health decisions.
                </p>
                <div className="flex items-end justify-between gap-2 mt-auto">
                  <button onClick={() => navigate("/doctors")} className="rounded-xl  bg-white text-teal-700 text-[11px] font-extrabold px-3 py-2 hover:bg-teal-50 transition-all cursor-pointer shadow-sm">
                    Consult Now →
                  </button>
                  <img src={doctorImage} alt="Doctor" className="w-22 h-22 object-contain drop-shadow-xl" />
                </div>
              </div>
            </div>
                ...
                <div className="flex items-start gap-2">
                  <div className="flex-1">
                    ...
                    <p className="text-slate-500 text-[11px] leading-relaxed">
                      Your health is our priority. Our team is always here to help you.
                    </p>
                  </div>
                  <img src={micsetImg} alt="Support" className="w-20 h-20 object-contain shrink-0 drop-shadow-sm" />
                </div>
              </div>
            </div>
          </div>
      ...
      {reviewModal && (
        <ReviewModal appointmentId={reviewModal.id ?? (reviewModal as any).appointment_id}
          doctorName={reviewModal.doctor?.name ?? "Doctor"} onClose={() => setReviewModal(null)}
          onSubmitted={() => {
            setAppointments(prev =>
              prev.map(a => a.id === reviewModal!.id ? { ...a, review_given: true } : a)
            );
            setReviewModal(null);
          }} />
      )}
